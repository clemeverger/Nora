    ...
    <div>
      ...
      <div className='bg-primary_light rounded-xl flex p-1 justify-between max-w-max align-middle mx-auto mb-8'>
        <button
          ...
          className={'px-3 py-2 rounded-xl text-primary_dark' + (page == 'calendar' ? ' bg-primary ' : '')}
        >
          Calendrier
        </button>
        <button
          value={'lock'}
          onClick={handleOnChange}
          className={'flex items-center px-3 py-2 rounded-xl text-primary_dark' + (page == 'lock' ? ' bg-primary ' : '')}
        >
          Analyse
          <Image
            src={'/lock.svg'}
